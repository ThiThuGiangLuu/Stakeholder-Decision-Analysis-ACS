...
acis_costbenefit <- function(x, varnames){
  ...
  #6.2.11.1 Average NPV for one animal raising household####
  ...
  #Running the model (previous version) ####
  ...
  #Run the model with handy results (not many folders but only 
  ...
  #We can try to plot some NPV or VIP scores
  #decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_results, 
                                      #vars = c("NPV_One_AnimalHH_Intervention4" ),
                                      #method = 'smooth_simple_overlay', 
                                      #base_size = 7)
  
  
  #pls_result_1 <- plsr.mcSimulation(object = mcSimulation_results,
                                    #resultName = "NPV_One_AnimalHH_Intervention4",
                                    #ncomp = 1)
  
  #plot_pls(pls_result_1, threshold = 0.8, input_table = test)
 
  
  #pls_result_2 <- plsr.mcSimulation(object = mcSimulation_results,
                                    #resultName = "NPV_Intervention4",
                                    #ncomp = 1)
  
  #plot_pls(pls_result_2, threshold = 0.8, input_table = test)
